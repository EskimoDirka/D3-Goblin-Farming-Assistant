...
- This app is meant to be used for Diablo III while in Windowed Full mode. It has a built in lock-cursor feature for dual monitor setups. I personally like to switch between Diablo III and my other monitor and it's a pain to always turn on/off the built-in lock cursor feature that Diablo has so I made one for this app that does this same feature while combat is active. So you can seemlessly switch between monitors when combat isn't running.
...
- Number 1 is used to teleport to the next location with logic failsafes in place. For the Xbox 360 S controller use the right trigger to teleport. If you use the hotkey to teleport and you're in a location that needs to be fully cleared, then it will prevent you from teleporting and a splash notification will appear in the Diablo III window telling you to first clear the area. For example, if you're in Cathedral Level 1 and try to use the hotkey to teleport next, you'll be blocked. You won't be able to teleport until you've reached Cathedral Level 3. This hotkey logic failsafe also applies to Cave of the Moon Clan Level 1, City of Caldeum, Sewers of Caldeum, Flooded Causeway, Western Channel Level 1, Eastern Channel Level 1, and Stinging Winds.
- If you use the teleport hotkey and aren't teleported to the next location that you wanted/intended to teleport to then the next time you use the teleport hotkey, it will verify if you actually teleported to that location, and if you haven't then it will teleport you to the correct location and update the overlay accordingly
...
- It has 3 different combat loops depending on which character you're using to farm. Monk, Demon Hunter, and Witch Doctor. It will automatically cast skills for all characters. All skills and skill placements on the action bar are listed below if you want an out-of-the-box experience for farming.
- It automatically builds momentum stacks for the Demon Hunter to 20 before it starts the combat loop then will shift + left click whenever your momentum stacks drop below 20 while combat is running (this is image recognition based, not timer based). For the Witch Doctor, it will scan if your Hex needs to be cast and auto cast so no need for any timers. It will also stop chicken mode to allow for clean teleports
- It also automatically scans which character you're using and uses that combat function so no need to change anything when swapping characters. To clarify, I've only built this for the female Monk, the male Demon Hunter, and the female Witch Doctor.
